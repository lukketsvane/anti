...
import { Navigation } from '@/app/components/nav'; // Make sure this is the correct import path
...
const Fawaris = () => {
    ...
    const toggleDarkMode = () => {
        ...
        localStorage.setItem('dark-mode', newDarkModeSetting.toString());
    ...
    return (
        <>
            ...
            <div className={`pt-24 flex flex-col md:flex-row items-start justify-center min-h-screen ${darkMode ? 'bg-gray-900' : 'bg-gray-100'}`}>
                ...
                <motion.div
                    className="w-full md:w-2/3 lg:w-3/4 mt-4 md:mt-0 p-4"
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 1 }}
                >
                    <div className={`overflow-y-auto max-h-screen shadow-lg rounded-lg p-6 ${darkMode ? 'bg-gray-800 text-gray-200' : 'bg-white text-gray-900'}`}>
